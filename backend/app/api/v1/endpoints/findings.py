...
async def get_findings(
    ...
    try:
        ...
        return [FindingService.finding_to_schema(finding) for finding in findings]
...
async def update_finding(
    ...
    Raises:
        HTTPException: If finding not found or update fails
    """
    try:
...
async def get_finding_detection(
    ...
    Raises:
        HTTPException: If finding not found or no detection exists
    """
    try:
        # Validate finding exists
        finding = await FindingService.get_finding_by_id(db, finding_id)
